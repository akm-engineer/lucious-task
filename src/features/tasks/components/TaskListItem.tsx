...
export function TaskListItem({ task, isDragging, dragHandleProps, onToggle, onEdit, onDelete, onOpen }: TaskListItemProps) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="shrink-0 cursor-grab active:cursor-grabbing text-slate-300 dark:text-slate-600 hover:text-slate-400 dark:hover:text-slate-400 opacity-0 group-hover:opacity-100 transition-all"
      >
      ...
      <div className="flex-1 min-w-0">
        ...
        {task.dueDate && (
          <span className={`inline-flex items-center gap-1 mt-1.5 text-[11px] font-medium ${overdue ? 'text-rose-500 dark:text-rose-400' : 'text-slate-400 dark:text-slate-500'}`}>
      ...
      {/* Actions */}
      <div className="flex items-center gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-200 shrink-0">
